...
if __name__=='__main__':
	...
	PG_rep = Bidirectional(GRU(embeddingSize, return_sequences = False, dropout=0.2), merge_mode='concat')(inputPG)
	QG_rep = Bidirectional(GRU(embeddingSize, return_sequences = False, dropout=0.2), merge_mode='concat')(inputQG)

	# print PG_rep.shape
	# print QG_rep.shape

	final_P = Concatenate()([P_rep,PG_rep])
	final_Q = Concatenate()([Q_rep,QG_rep])
	...
	model.compile(loss = c_loss, optimizer=Adam(lr=learn_rate), metrics=['acc'])
	model.summary()
	...
	print("Siamese with Glove Embeddings Accuracy  : %0.2f%% "% (100 * acc))
