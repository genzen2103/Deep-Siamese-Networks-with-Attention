...
def cosine_dist(vects):
	return Activation('relu')(dot(vects,axes=1,normalize=True))
...
if __name__=='__main__':
	...
	similarity_matrix = Dot(2)([inputPG,inputQG]) 								# 1 x m x n
	similarity_matrix_transpose =  Permute((2,1))  (similarity_matrix) 			# 1 x  n x m

	P_atten = Activation('softmax')(similarity_matrix_transpose)				# 1 x n x m
	Q_atten = Activation('softmax')(similarity_matrix)							# 1 x m x n
	

	P_context = Dot(1)([P_atten,inputQG])										# 1 x m x 300
	Q_context = Dot(1)([Q_atten,inputPG])   									# 1 x n x 300

	fixed_P_rep_flat = Reshape((M*glove_emb,)) (P_context)
	fixed_Q_rep_flat = Reshape((N*glove_emb,))  (Q_context)

#	print(fixed_P_rep_flat.shape,fixed_Q_rep_flat.shape)

	final_P = Concatenate()([P_rep,fixed_P_rep_flat])
	final_Q = Concatenate()([Q_rep,fixed_Q_rep_flat])

	distance = Lambda(cosine_dist, output_shape=cos_dist_output_shape)([final_P, final_Q])
	model = Model(outputs=distance,inputs=[inputP, inputQ,inputPG,inputQG])
	model.compile(loss = c_loss, optimizer=Adam(lr=learn_rate,decay=0.000001), metrics=['acc'])
	model.summary()
	# Parameters
	params = {'dim': 11522,
	...
	acc1 = model.evaluate_generator(generator=test_generator, steps=None, max_queue_size=10, workers=12, use_multiprocessing=True)
	print("Accuracy1 : %0.2f%% "% (100 * acc1[1]))
